...
ConstControlResult Manipulator::constControl(VectorXd q, Matrix4d taskHtm, vector<GeometricPrimitives*> obstacles,
                                             ConstControlResult* oldControlStruct, ConstControlParam param,
                                             Matrix4d customHtmWorldToBase)
{
  ...
  modTask[3] = param.kori * sqrt(max(tr.task[3],0));
  modTask[4] = param.kori * sqrt(max(tr.task[4],0));
  modTask[5] = param.kori * sqrt(max(tr.task[5],0));
...
ConstControlResult Manipulator::accelConstControl(Matrix4d taskHtm, VectorXd qdot,
                                                  vector<GeometricPrimitives*> obstacles,
                                                  ConstControlResult* oldControlStruct, AccelConstControlParam param,
                                                  VectorXd q, Matrix4d customHtmWorldToBase)
{
  ConstControlResult ccr;
  ...
  for (int i = 0; i < obstacles.size(); i++)
  {
    dloOld = (oldControlStruct == NULL) ? NULL : oldControlStruct->distancesObjResult[i];
    dlo = this->computeDistToObj(obstacles[i], q, customHtmWorldToBase, &dloOld, param.tol, param.h, param.maxDistAABB);
    dloNext = this->computeDistToObj(obstacles[i], q + qdot * param.dt, customHtmWorldToBase, &dlo, param.tol, param.h,
                                     param.maxDistAABB);
    ...
    for (int j = 0; j < dlo.noElements; j++)
    {
      ...
      if (dlorNext != NULL)
      {
        ...
        // ROS_INFO_STREAM("CC for obs " << i << ", element " << j << ": " << coriolisCentrifugal);
        // ROS_INFO_STREAM("gradDot for obs " << i << ", link " << dlo[j].linkNumber << ": "
        //                                   << round(1000 * ) / 1000);

        ccr.maxNormdotJac = max(ccr.maxNormdotJac, gradDot.norm());
  ...
  if (param.considerAutoCollision)
  {
    ...
    dra = this->computeDistAuto(q, customHtmWorldToBase, &draOld, param.tol, param.h, param.maxDistAABB);
    draNext =
        this->computeDistAuto(q + qdot * param.dt, customHtmWorldToBase, &dra, param.tol, param.h, param.maxDistAABB);
  ...
  // {
  //   if (b[K] > b[imax]) imax = K;
  // }

  // ROS_INFO_STREAM("b = " << b.transpose());
  // ROS_INFO_STREAM("imax = " << imax);
  // ROS_INFO_STREAM("bmax = " << b[imax]);
  // MatrixXd Ar = A.block<1, 7>(imax, 0);
  // ROS_INFO_STREAM("Amax = " << Ar);
