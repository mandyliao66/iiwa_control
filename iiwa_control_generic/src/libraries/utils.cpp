...
string Utils::printMatrixPython(MatrixXd M)
{
    string str = "[";
    for(int i=0; i < M.rows(); i++)
        str+= printVector(M.row(i)) + ((i<M.rows()-1)? ',' : ' ');

    return str+"]";    
}

void Utils::sendToSimulation(ofstream* simFile, double time, VectorXd q)
{
    *simFile << "iiwa.add_ani_frame("<<time<<","<<printVector(q)<<")"<<std::endl;
}